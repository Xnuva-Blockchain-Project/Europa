...
void CWallet::BlockDisconnected(const std::shared_ptr<const CBlock>& pblock) {
    ...
    for (const CTransactionRef& ptx : pblock->vtx) {
        if (ptx->IsCoinBase() && IsMine(*ptx))
        {
            WalletLogPrintf(
                "REORG: coinbase reward %s removed from active chain; "
                "reward remains tracked until block becomes active again\n",
...
CAmount CWallet::GetInactiveMinedBalance() const
{
    CAmount nTotal = 0;
    auto locked_chain = chain().lock();
    LOCK(cs_wallet);
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        // Coinbase was previously recorded in a block, but that block is not
        // currently part of the active chain. Retain the value for historical
        // visibility only; it remains non-spendable.
        if (!wtx.IsCoinBase() || wtx.hashUnset() || wtx.nIndex < 0 ||
            wtx.GetDepthInMainChain(*locked_chain) != 0) {
            continue;
        }

        nTotal += GetCredit(*wtx.tx, ISMINE_SPENDABLE);
