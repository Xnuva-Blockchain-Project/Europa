        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated europa to."},
            ...
            + "If you are running the europa core wallet, you can get a new address to send the newly generated europa to with:\n"
...
static UniValue getmininginfo(const JSONRPCRequest& request)
{
    ...
    obj.pushKV("pow_algorithm",    "scrypt");
    obj.pushKV("target_spacing",   Params().GetConsensus().nPowTargetSpacing);
    obj.pushKV("target_timespan",  Params().GetConsensus().nPowTargetTimespan);
    obj.pushKV("difficulty_adjustment_interval",
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Europa is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Europa is downloading blocks...");
