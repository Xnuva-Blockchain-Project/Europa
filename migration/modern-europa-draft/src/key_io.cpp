...
public:
    ...
    {
        // EUROPA WALLET COMPATIBILITY: historical encoder uses SCRIPT_ADDRESS2.
    ...
    std::string operator()(const WitnessV1Taproot& tap) const
    {
        ...
        return bech32::Encode(
            m_params.LegacyWitnessUnknownBech32() ? bech32::Encoding::BECH32 : bech32::Encoding::BECH32M,
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    if (!is_bech32 && DecodeBase58Check(str, data, 21)) {
        ...
        // If the prefix of data matches a script or pubkey prefix, the length must have been wrong
    ...
    if (dec.encoding == bech32::Encoding::BECH32 || dec.encoding == bech32::Encoding::BECH32M) {
        ...
        if (version != 0) {
            if (params.LegacyWitnessUnknownBech32()) {
                if (dec.encoding != bech32::Encoding::BECH32) {
                    error_str = "Europa legacy version 1+ witness address must use Bech32 checksum";
                    return CNoDestination();
                }
            } else if (dec.encoding != bech32::Encoding::BECH32M) {
